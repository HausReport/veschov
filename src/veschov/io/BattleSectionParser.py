...
class BattleSectionParser(AbstractSectionParser):
    ...
    def _normalize_combat_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        renamed = cleaned.rename(columns=self.COMBAT_COLUMN_RENAMES, inplace=False)
        renamed = self._coerce_numeric_columns(renamed, self.NORMALIZED_NUMERIC_COLUMNS)
        renamed = self._coerce_yes_no_columns(renamed, self.COMBAT_BOOLEAN_COLUMNS)
        ...
        ordered = [column for column in self.COLUMN_ORDER if column in renamed.columns]
        extras = [column for column in renamed.columns if column not in ordered]
        return renamed.loc[:, ordered + extras]
