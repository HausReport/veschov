...
def render_raw_damage_report() -> None:
    """Render the raw damage (pre-mitigation) report."""
    st.markdown(
        "Raw Damage by Type shows pre-mitigation totals (Normal vs Isolytic), split by whether the "
        "hit was critical."
    )

    df = render_sidebar_combat_log_upload(
        "Raw Damage by Type",
        "Upload a battle log to visualize raw damage (normal vs isolytic, crit vs non-crit).",
    ...
    view_by = select_view_by("raw_damage_view_by")
    ...
    fig = px.area(
        ...
        y="amount",
        color="series_name",
        title=f"Raw Damage by Type — {battle_filename}",
        category_orders={
            "series_name": [
    ...
    st.caption("Stacks show how much raw damage was logged for each damage type and crit state.")
